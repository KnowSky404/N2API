...
func TestCreateRequestLogSQLIncludesProviderAccountAttribution(t *testing.T) {
	...
	for _, want := range []string{"provider_account_id", "provider_account_type", "model", "input_tokens", "total_tokens", "usage_source", "estimated_cost_microusd", "pricing_snapshot"} {
