...
func TestCreateRequestLogSQLIncludesProviderAccountAttribution(t *testing.T) {
	...
	for _, want := range []string{"provider_account_id", "provider_account_type", "provider_account_name", "model", "session_id", "input_tokens", "total_tokens", "usage_source", "estimated_cost_microusd", "pricing_snapshot", "gateway_attempt_count", "gateway_fallback_count"} {
	...
	for _, want := range []string{"entry.GatewayAttemptCount", "entry.GatewayFallbackCount"} {
