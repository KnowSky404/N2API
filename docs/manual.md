...
`1.26.6` on Alpine `3.23`, Alpine `3.23.5`, and PostgreSQL `18.4` on Alpine
...
- Use the account row to set a display name, priority, and **Scheduling preference**. OAuth account creation also lets you choose whether the account should be enabled after login.
...
- Set **Bulk priority**, **Bulk scheduling preference**, or **Bulk max concurrency**, then use **Apply scheduling** to update selected provider accounts together; bulk priority, bulk scheduling preference, and bulk max concurrency fields use the same validation as each account row.
...
- API key and Routing Pool management responses use signed, filter-bound keyset cursors with a default page size of 50 and a maximum of 100. The admin UI loads additional pages explicitly and keeps partial rows visible when a later page fails. The API Keys page supports local search and status filtering by name, prefix, model policy, selected model, active/disabled/deleted status, and limiter state, plus routing-pool filtering, across the rows currently loaded; bulk actions submit only the selected loaded IDs and never imply selection of unloaded pages.
...
- Use **Test account** before sending client traffic through an account. The action probes one provider account with its current OAuth token or API upstream key. Only a 2xx response confirms recovery and clears local failure status; network errors and every non-2xx response record a failed test without a recovery transition. The account row keeps the last test status, last test time, and last test error so manual checks remain visible after refresh. Each probe also writes provider account test history; use the Providers page **History action** to expand **Recent test history**, or fetch the same data from `GET /api/admin/provider-accounts/{id}/test-results`.
- Gateway account selection records only an attempt timestamp. It does not clear account health. A gateway request confirms account recovery only when its final upstream response is 2xx; redirects, client/server errors, authorization failures, network failures, and configured error passthrough do not recover the account. When fallback succeeds, only the account that produced the final 2xx response recovers.
...
- Stateful Responses account affinity stores only a domain-separated HMAC of each response ID, scoped to the client key's primary routing pool. `N2API_RESPONSE_AFFINITY_TTL_DAYS` defaults to `30` days. Expired-row cleanup remains disabled unless `N2API_RESPONSE_AFFINITY_RETENTION_RUNNER_ENABLED=true`; its interval and single-cycle delete bound are controlled by `N2API_RESPONSE_AFFINITY_RETENTION_INTERVAL_SECONDS` (`300`-`604800`) and `N2API_RESPONSE_AFFINITY_RETENTION_BATCH_SIZE` (`100`-`10000`). Authenticated `/api/admin/health` includes the runner's last start, success, failure, lock skip, stable error code, and delete count.
...
- Use **Pause scheduling** when a healthy account should stop receiving traffic for a short window. Set **Pause duration seconds** on the Provider accounts page before clicking the action; it temporarily opens the account circuit for that window without disabling or deleting the account. Paused and rate-limited rows show the remaining scheduling block in the status column. Use **Reset local status** to clear the pause early. Reset is an explicit operator override: when it clears unhealthy fields, N2API records both the status-reset audit event and a runtime recovery event even without a provider 2xx.
- Selected provider accounts can be paused and reset together. Use **Pause selected** to apply the configured **Pause duration seconds** to every selected account, or **Reset selected** to apply the same explicit operator override to each selected account. Prefer **Test selected** when provider-confirmed recovery is required.
...
- The UI calls the compatibility field **Scheduling preference**; the JSON/API identifier remains `loadFactor`. Within the same priority scope, higher values form a strict descending preference tier, not a proportional request weight. A continuously eligible account in a higher scheduling preference tier is considered before every account in a lower tier and can therefore receive every new non-sticky selection. Keep weak or quota-sensitive accounts at scheduling preference `1`; raise the value only when that strict preference is intended.
- Within an exactly equal priority and scheduling preference tier, accounts without a recent error are considered before accounts with one, followed by least-recently-used time and account ID. A stored sticky binding is reused while its account remains schedulable. For a new session, sticky FNV hashing only changes order inside the highest tier with exactly equal pool priority (when scoped), global account priority, scheduling preference, and recent-error state; it never promotes a lower scheduling preference tier.
