...
The client key must be bound to a routing pool. The configured model must be
visible to the key and enabled on at least one schedulable provider account in
that pool or its explicit fallback chain.
...
Configure model capability on each provider account row. The API Keys page controls routing-pool binding and each client's `all` or `selected` model policy; these settings do not grant capability to accounts that do not list that model. The gateway `defaultModel` is used only when a POST request omits `model`, and the injected model must still be routable in that key's bound pool or explicit fallback chain.
...
Provider-account model configuration is the source of truth for model capability. Accounts with no configured models remain available for editing but cannot serve model-routed requests.
...
Client API keys retain `all` and `selected` model policies, but model access is fail-closed until the key is bound to a routing pool. Within the bound pool and its explicit fallback chain, `all` keys see every currently routable configured model, while `selected` keys see the intersection of their selected models and those routable models. A selected model must still have at least one enabled healthy provider account in that chain.

Routing pools partition provider accounts for different agents, devices, or risk profiles. Every usable API key must be bound to one routing pool from the API Keys page; a key without a pool has no available models and cannot route model requests. A bound key only schedules accounts in that pool and its explicit fallback chain, including sticky session bindings scoped to that chain. Pool membership priority is evaluated before the account's global priority inside that pool, so the same provider account can be ranked differently for different client keys. Missing or deleted pools fail closed with `routing_pool_unavailable`, empty enabled pools fail closed with `routing_pool_empty`, and Request Logs retain the routing pool name/id for attribution.
...
`/v1/models` returns the models exposed to the authenticated API key. An unbound key receives `200` with an empty model list; a model-routed POST made with that key fails closed with `503` and the error code `routing_pool_required`. For a bound key, a model must be configured and enabled on at least one schedulable account in the key's routing-pool fallback chain and visible under the key's `all` or `selected` policy. Model-routed POST traffic is sent only to accounts in that chain that explicitly support the requested or defaulted model. Connected accounts with no configured models remain visible in admin but do not receive model-routed POST traffic.
