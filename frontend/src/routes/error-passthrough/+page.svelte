  ...
  <div class="space-y-6">
    <section class="rounded-lg border border-[#ededed] bg-white p-6">
      <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <button class="rounded-lg bg-[#0d0d0d] px-4 py-2 text-sm font-medium text-white" onclick={() => { resetForm(); showForm = true; }}>New rule</button>
      </div>
    </section>
      ...
      <section class="rounded-lg border border-[#ededed] bg-white p-6">
        ...
        <form class="mt-4 space-y-4" onsubmit={handleSubmit}>
          ...
          <div class="flex items-center gap-3">
            <button class="rounded-lg bg-[#0d0d0d] px-4 py-2 text-sm font-medium text-white disabled:opacity-60" disabled={errorPassthroughRules.saving} type="submit">{errorPassthroughRules.saving ? 'Saving' : editingId ? 'Save' : 'Create'}</button>
            <button class="rounded-lg border border-[#e5e5e5] bg-white px-4 py-2 text-sm font-medium text-[#0d0d0d]" type="button" onclick={resetForm}>Cancel</button>
          </div>
        </form>
      </section>
    {/if}
    ...
    <section class="rounded-lg border border-[#ededed] bg-white">
      {#if errorPassthroughRules.loading}
        <p class="p-6 text-sm text-[#6e6e6e]">Loading...</p>
              ...
              {#each errorPassthroughRules.items as rule}
                <tr class="hover:bg-[#fafafa]">
                  ...
                  <td class="px-4 py-3 text-right">
                    <button class="rounded-md px-2.5 py-1.5 text-sm font-medium text-[#0d0d0d] hover:bg-[#f5f5f5]" onclick={() => edit(rule)}>Edit</button>
                    <button class="ml-1 rounded-md px-2.5 py-1.5 text-sm font-medium text-red-700 hover:bg-red-50" onclick={() => handleDelete(rule.id, rule.pattern)}>Delete</button>
                  </td>
                </tr>
              {/each}
            </tbody>
