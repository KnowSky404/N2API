...
case "${mode}" in
  ...
  control-connections)
    ...
    n2api_run_command bash -c '
      ...
      go test -count=1 -run "Test(PostgresConnection|PostgresPool|InstanceLock|MigrationLock|SystemEventSubscription|APIKeyAuthentication)" ./internal/store
      go test -race -count=1 -run "TestAPIKeyAuthentication" ./internal/store
